...
class Epimodel_esr(Model):
    def __init__(self, model_data: DataFrame):
        ...
        self.datacollector = DataCollector(
            agent_reporters={
                "State": "state",
                "infection_src_type": "infection_src_type",
                "infection_src_venue": "infection_src_venue",
            }
        )
        ...
        for i, person_loc_id in enumerate(unique_ids):

            proc_diary_locations = list(
                syspop_diary[syspop_diary.id == person_loc_id].location.values
            )
            ...
            for proc_diary_location in proc_diary_locations:
                ...
                proc_imms = syspop_healthcare[
                    syspop_healthcare.id == int(person_loc_id.split("_")[0])
                ]["mmr"].values
                proc_type = syspop_diary[
                    (syspop_diary.id == person_loc_id)
                    & (syspop_diary.location == proc_diary_location)
                ].type.values
                proc_type_location = syspop_diary[
                    (syspop_diary.id == person_loc_id)
                    & (syspop_diary.location == proc_diary_location)
                ].location.values
                ...
                person_attr = syspop_base[
                    syspop_base.id == int(person_loc_id.split("_")[0])
                ]
    ...
    def initial_infection(
        ...
        for i, proc_infection in enumerate(initial_infection):

            initial_n = list(proc_infection.keys())[0]

            selected_agents = filter_initial_agents(
                person_agents, initial_infection_cfg
            )
